...
export default function FollowingPage() {
  ...
  useEffect(() => {
    if (handle) {
      loadProfileAndFollowing(handle);
    }
  }, [handle]);
  ...
  const loadProfileAndFollowing = async (userHandle: string) => {
    try {
      setIsLoading(true);
      ...
      setPagination({
        page: followingResponse.pagination.page,
        hasMore: followingResponse.pagination.hasMore,
        total: followingResponse.pagination.total,
      });
    } catch (error) {
                    ...
                    <Link href={`/u/${user.handle}`}>
                      <Avatar className="w-12 h-12 border-2 border-border hover:border-brand-primary/50 transition-colors cursor-pointer">
                        ...
                        <AvatarFallback className="bg-gradient-to-br from-brand-primary to-brand-magenta text-white">
                          {user.isAnonymous ? "A" : user.name[0]}
                        </AvatarFallback>
                      </Avatar>
                    </Link>
                    ...
                    <div className="flex-1 min-w-0">
                      <Link href={`/u/${user.handle}`} className="block">
                        ...
                        <div className="flex gap-4 text-xs text-muted-foreground">
                          <span>{user.stats.imageCount} images</span>
